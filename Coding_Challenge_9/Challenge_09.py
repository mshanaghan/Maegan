...
input_shp = r'C:\ShanaghanNRS528\NRS_528_Coding_Challenges\Challenge_09\RI_Forest_Health_Works_Project%3A_Points_All_Invasives.shp'
...
expression_2 = arcpy.AddFieldDelimiters(input_shp, "photo") + " = ''"
# print expression_2
...
with arcpy.da.SearchCursor(input_shp, fields, expression_2) as cursor:
    for row in cursor:
        print(u'{0}, {1}'.format(row[0], row[1]))
        count_2 = count_2 + 1
    print("There are" +" "+ str(count_2) +" "+ "records without pictures.")
...
with arcpy.da.SearchCursor(input_shp, field) as cursor:
    for row in cursor:
        if row[0] not in SpeciesList:
            SpeciesList.append(row[0])
    print("There are" + " " + str(len(SpeciesList)) + " " + "unique species in this dataset.")
